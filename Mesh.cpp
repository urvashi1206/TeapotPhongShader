...
void Mesh::draw1(mat4 viewMat, mat4 projMat, vec3 lightPos1, vec3 lightPos2, float time, vec3 eye) {
	...
	modelMat = scale(m, vec3(0.5f, 0.5f, 0.5f));
	...
	shaderProg.setFloat3V("viewPos", 1, value_ptr(eye));
...
void Mesh::draw2(mat4 viewMat, mat4 projMat, vec3 lightPos1, vec3 lightPos2, float time, vec3 eye) {
	...
	glUseProgram(shaderProg.id);
	mat4 m = translate(mat4(1.0), vec3(3.0f, 2.0f, 0.0f));
	modelMat = scale(m, vec3(0.5f, 0.5f, 0.5f));
	shaderProg.setMatrix4fv("modelMat", 1, value_ptr(modelMat));
	shaderProg.setMatrix4fv("viewMat", 1, value_ptr(viewMat));
	shaderProg.setMatrix4fv("projMat", 1, value_ptr(projMat));
